...
# 读取原始数据
df0 = pd.read_excel('OLT基础信息20180724.xls', engine = 'xlrd',
...
### 判断'下接盒式OLT'和'归属框式OLT' 字段是否准确
#df_wly['下接盒式OLT'].fillna('缺失数据', inplace = True)
#df_wly['归属框式OLT'].fillna('缺失数据', inplace = True)
#df_wlyhs1 = df_wly[(df_wly['下接盒式OLT'] != '缺失数据')]
#df_wlyhs2 = df_wly[(df_wly['归属框式OLT'] != '缺失数据')]
#df_wlyhs3 = pd.merge(df_wlyhs1,df_wlyhs2,how = 'outer',left_on='OLT名称',right_on='归属框式OLT')
#df_wlyhs3.to_excel('OLT扩容异常数据3网络域%s.xls' % datetime.datetime.now().strftime("%y-%m-%d-%H-%M"),sheet_name='网络域相关字段异常数据',index = 0)

# 梳理“原上联”字段
df_wlyysl = df_wly[['OLT名称','原上联','网络域类型','归属框式OLT','归属BNG','归属BNG2','归属SW']]
if df_wlyysl.where(df_wlyysl['网络域类型'] == 'BNG'):
    df_wlyysl['原上联核对'] = df_wlyysl['归属框式OLT']
else:
    pass
...
# 保存数据到Excel
df_ghxq2.to_excel('OLT扩容异常数据3规划需求%s.xls' % datetime.datetime.now().strftime("%y-%m-%d-%H-%M"),sheet_name='未有规划异常数据',index = 0)
'''
